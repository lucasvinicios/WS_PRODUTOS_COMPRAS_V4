# import pandas as pd
# import io
# from openpyxl.styles import PatternFill
# from openpyxl.utils import get_column_letter

# def realizar_analise_reversa_acumulativa(file_path):
#     """
#     Realiza a análise de preços de produtos lendo diretamente de um arquivo Excel,
#     encontrando o item mais barato em cada mercado para a cesta de compras.
#     """
#     mercados_data = {}
    
#     excel_file = pd.ExcelFile(file_path)
#     sheet_names = excel_file.sheet_names
    
#     for sheet_name in sheet_names:
#         df = pd.read_excel(file_path, sheet_name=sheet_name)
        
#         # A lógica para extrair os dados de cada aba permanece a mesma
#         df.set_index('Produto', inplace=True)
#         nome_mercado_curto = df.columns[0]
#         print("Aqui")
#         print(df)
#         frete = df.loc['Frete', nome_mercado_curto]
        
#         df_produtos = df.drop(index=['Frete', 'Valor Mínimo', 'Valor Total', 'Total Baratos + Frete'], errors='ignore')

#         mercados_data[sheet_name] = {
#             'df': df_produtos,
#             'frete': frete,
#             'nome_curto': nome_mercado_curto
#         }

#     resultados_finais = []
    
#     custos_totais = pd.DataFrame()
#     for nome_arquivo_completo, data in mercados_data.items():
#         df_temp = data['df'].copy()
#         df_temp['Custo Total'] = df_temp.iloc[:, 0] + data['frete']
#         custos_totais[nome_arquivo_completo] = df_temp['Custo Total']
    
#     for produto in custos_totais.index:
#         custos_ordenados = custos_totais.loc[produto].dropna().sort_values()
        
#         if not custos_ordenados.empty:
#             mercado_mais_barato_key = custos_ordenados.index[0]
#             preco_produto = mercados_data[mercado_mais_barato_key]['df'].loc[produto].iloc[0]
#             frete = mercados_data[mercado_mais_barato_key]['frete']
#             custo_total = preco_produto + frete
            
#             resultados_finais.append({
#                 'Produto': produto,
#                 'Supermercado Recomendado': mercado_mais_barato_key,
#                 'Preço do Produto': preco_produto,
#                 'Frete': frete,
#                 'Custo Total': custo_total,
#             })
            
#     return pd.DataFrame(resultados_finais).sort_values(by='Produto'), mercados_data

# def gerar_aba_comparacao_dinamica(writer, mercados_data, analise_final_df):
#     """
#     Gera uma aba de comparação para os três supermercados mais baratos
#     com a fórmula de economia explícita do usuário.
#     """
#     # 1. Identifica os 3 supermercados com a menor cesta total
#     custos_totais_mercados = {}
#     for nome_completo, data in mercados_data.items():
#         custo_total = data['df'].iloc[:, 0].sum() + data['frete']
#         custos_totais_mercados[nome_completo] = custo_total
    
#     top_3_mercados = sorted(custos_totais_mercados.items(), key=lambda item: item[1])[:3]
#     top_3_nomes_completos = [item[0] for item in top_3_mercados]
    
#     # 2. Filtra os dados apenas para esses 3 supermercados
#     mercados_selecionados = {k: v for k, v in mercados_data.items() if k in top_3_nomes_completos}
    
#     # 3. Coleta os preços e fretes
#     precos_comparativos = pd.DataFrame()
#     fretes = {}
#     for nome_completo, data in mercados_selecionados.items():
#         precos_comparativos[data['nome_curto']] = data['df'].iloc[:, 0]
#         fretes[data['nome_curto']] = data['frete']
    
#     precos_comparativos.loc['Frete'] = pd.Series(fretes)

#     # 4. Calcula o "Total Economizado" usando a fórmula explícita
#     economias_dinamica = {}

#     todos_os_precos = pd.DataFrame()
#     for nome_completo, data in mercados_data.items():
#         todos_os_precos[data['nome_curto']] = data['df'].iloc[:, 0]

#     for mercado_curto_atual in precos_comparativos.columns:
#         frete_mercado_atual = fretes[mercado_curto_atual]
#         soma_das_diferencas = 0

#         for produto in todos_os_precos.index:
#             precos_produto_ordenados = todos_os_precos.loc[produto].dropna().sort_values()[:3]
            
#             if len(precos_produto_ordenados) >= 2:
#                 mercado_mais_barato_curto = precos_produto_ordenados.index[0]
                
#                 if mercado_mais_barato_curto == mercado_curto_atual:
#                     preco_mais_barato = precos_produto_ordenados.iloc[0]
#                     preco_segundo_mais_barato = precos_produto_ordenados.iloc[1]
#                     soma_das_diferencas += (preco_segundo_mais_barato - preco_mais_barato)

#         economias_dinamica[mercado_curto_atual] = soma_das_diferencas - frete_mercado_atual
    
#     precos_comparativos.loc['Total Economizado'] = pd.Series(economias_dinamica)

#     # 5. Escreve os dados na planilha
#     nome_aba = 'Comparativo Dinâmico'
#     precos_comparativos.index.name = 'Produto'
#     precos_comparativos.to_excel(writer, sheet_name=nome_aba, index=True)
    
#     # 6. Aplica a formatação condicional e de valores
#     workbook = writer.book
#     worksheet = workbook[nome_aba]

#     verde_fill = PatternFill(start_color='63BE7B', end_color='63BE7B', fill_type='solid')
#     azul_fill = PatternFill(start_color='4472C4', end_color='4472C4', fill_type='solid')
    
#     num_mercados = len(precos_comparativos.columns)
    
#     for row_idx, produto in enumerate(precos_comparativos.index, start=2):
#         if produto in ['Frete', 'Total Economizado']:
#             continue
            
#         valores = [worksheet.cell(row=row_idx, column=i).value for i in range(2, num_mercados + 2)]
        
#         valores_numericos = [v for v in valores if isinstance(v, (int, float))]
        
#         if valores_numericos:
#             valores_ordenados = sorted(valores_numericos)
            
#             min_val = valores_ordenados[0]
#             for col_idx in range(2, num_mercados + 2):
#                 cell = worksheet.cell(row=row_idx, column=col_idx)
#                 if cell.value == min_val:
#                     cell.fill = verde_fill
            
#             if len(valores_ordenados) >= 2:
#                 second_min_val = valores_ordenados[1]
#                 for col_idx in range(2, num_mercados + 2):
#                     cell = worksheet.cell(row=row_idx, column=col_idx)
#                     if cell.value == second_min_val and cell.fill != verde_fill:
#                         cell.fill = azul_fill
                        
#     for col in range(2, num_mercados + 2):
#         for row in range(2, len(precos_comparativos.index) + 2):
#             cell = worksheet.cell(row=row, column=col)
#             cell.number_format = '"R$" #,##0.00'
            
#     for col in worksheet.columns:
#         max_length = 0
#         column_letter = get_column_letter(col[0].column)
#         for cell in col:
#             try:
#                 if len(str(cell.value)) > max_length:
#                     max_length = len(str(cell.value))
#             except:
#                 pass
#         adjusted_width = (max_length + 2)
#         worksheet.column_dimensions[column_letter].width = adjusted_width

#     print("Aba 'Comparativo Dinâmico' gerada com sucesso e formatada.")


# def gerar_excel_final(analise_final_df, mercados_data, nome_arquivo='analise_supermercados_final.xlsx'):
#     """
#     Gera um arquivo Excel com abas para cada supermercado recomendado e um resumo.
#     """
#     custos_totais_com_nomes_curtos = pd.DataFrame()
#     for nome_arquivo_completo, data in mercados_data.items():
#         nome_mercado_curto = data['nome_curto']
#         df_temp = data['df'].copy()
#         df_temp['Custo Total'] = df_temp.iloc[:, 0].sum() + data['frete']
#         custos_totais_com_nomes_curtos[nome_mercado_curto] = df_temp['Custo Total']
    
#     economias = {}
#     for produto in custos_totais_com_nomes_curtos.index:
#         custos_ordenados = custos_totais_com_nomes_curtos.loc[produto].sort_values()
#         custo_melhor = custos_ordenados.iloc[0]
#         try:
#             custo_segundo_melhor = custos_ordenados.iloc[1]
#             economia_calculada = custo_segundo_melhor - custo_melhor
#             economias[produto] = economia_calculada
#         except IndexError:
#             economias[produto] = 0

#     mercados_recomendados_keys = analise_final_df['Supermercado Recomendado'].unique()
    
#     mercados_recomendados_ordenados = sorted(
#         mercados_recomendados_keys,
#         key=lambda x: int(x.split(' ')[1]) if 'TOP' in x else float('inf')
#     )

#     with pd.ExcelWriter(nome_arquivo, engine='openpyxl') as writer:
        
#         gerar_aba_comparacao_dinamica(writer, mercados_data, analise_final_df)

#         for nome_mercado_completo in mercados_recomendados_ordenados:
#             data = mercados_data[nome_mercado_completo]
#             df_mercado = data['df'].copy()
#             frete = data['frete']

#             df_mercado['Custo Total (c/ Frete)'] = df_mercado.iloc[:, 0] + frete
#             df_mercado['Economia (vs 2º Melhor)'] = 0.00
            
#             for _, row in analise_final_df[analise_final_df['Supermercado Recomendado'] == nome_mercado_completo].iterrows():
#                 produto = row['Produto']
#                 df_mercado.loc[produto, 'Economia (vs 2º Melhor)'] = economias.get(produto, 0)
            
#             total_economizado = df_mercado['Economia (vs 2º Melhor)'].sum()

#             df_frete = pd.DataFrame({'Frete': frete, 'Custo Total (c/ Frete)': ''}, index=['Frete'])
#             df_frete.index.name = 'Produto'
            
#             df_total = pd.DataFrame({'Economia (vs 2º Melhor)': total_economizado}, index=['Total Economizado'])
#             df_total.index.name = 'Produto'

#             df_final = df_mercado[['Custo Total (c/ Frete)', 'Economia (vs 2º Melhor)']].copy()
#             df_final.insert(0, 'Preço Unitário', df_mercado.iloc[:, 0])
            
#             df_final = pd.concat([df_final, df_frete, df_total])
            
#             df_final = df_final.drop(columns=['Frete'], errors='ignore')
            
#             nome_aba = nome_mercado_completo.replace(' ', ' - ', 1)
            
#             df_final.to_excel(writer, sheet_name=nome_aba, index=True)
#             print(f"Aba '{nome_aba}' gerada com sucesso. Total economizado: R$ {total_economizado:.2f}")

# # --- Seu código para execução, agora ajustado ---

# arquivos_do_excel = "Resultados\\analise_supermercados_2025-08-27.xlsx"

# try:
#     # 1. Executa a análise lendo diretamente do arquivo Excel
#     analise_completa, mercados_dados = realizar_analise_reversa_acumulativa(arquivos_do_excel)

#     # 2. Gera o novo arquivo Excel
#     gerar_excel_final(analise_completa, mercados_dados)
    
# except FileNotFoundError:
#     print(f"Erro: O arquivo '{arquivos_do_excel}' não foi encontrado.")
# except Exception as e:
#     print(f"Ocorreu um erro: {e}. Linha de erro: {e.__traceback__.tb_lineno}")

# import pandas as pd
# import io
# from openpyxl.styles import PatternFill
# from openpyxl.utils import get_column_letter
# from itertools import combinations

# def realizar_analise_reversa_acumulativa(file_path):
#     """
#     Realiza a análise de preços de produtos lendo diretamente de um arquivo Excel,
#     encontrando o item mais barato em cada mercado para a cesta de compras.
#     """
#     mercados_data = {}
    
#     excel_file = pd.ExcelFile(file_path)
#     sheet_names = excel_file.sheet_names
    
#     for sheet_name in sheet_names:
#         df = pd.read_excel(file_path, sheet_name=sheet_name)
        
#         # A lógica para extrair os dados de cada aba permanece a mesma
#         df.set_index('Produto', inplace=True)
#         nome_mercado_curto = df.columns[0]
#         frete = df.loc['Frete', nome_mercado_curto]
#         df_produtos = df.drop(index=['Frete', 'Valor Mínimo', 'Valor Total', 'Total Baratos + Frete'], errors='ignore')

#         mercados_data[sheet_name] = {
#             'df': df_produtos,
#             'frete': frete,
#             'nome_curto': nome_mercado_curto
#         }

#     resultados_finais = []
    
#     custos_totais = pd.DataFrame()
#     for nome_arquivo_completo, data in mercados_data.items():
#         df_temp = data['df'].copy()
#         df_temp['Custo Total'] = df_temp.iloc[:, 0].sum() + data['frete']
#         custos_totais[nome_arquivo_completo] = df_temp['Custo Total']
    
#     for produto in custos_totais.index:
#         custos_ordenados = custos_totais.loc[produto].dropna().sort_values()
        
#         if not custos_ordenados.empty:
#             mercado_mais_barato_key = custos_ordenados.index[0]
#             preco_produto = mercados_data[mercado_mais_barato_key]['df'].loc[produto].iloc[0]
#             frete = mercados_data[mercado_mais_barato_key]['frete']
#             custo_total = preco_produto + frete
            
#             resultados_finais.append({
#                 'Produto': produto,
#                 'Supermercado Recomendado': mercado_mais_barato_key,
#                 'Preço do Produto': preco_produto,
#                 'Frete': frete,
#                 'Custo Total': custo_total,
#             })
            
#     return pd.DataFrame(resultados_finais).sort_values(by='Produto'), mercados_data

# def gerar_aba_comparacao_dinamica(writer, mercados_data, analise_final_df):
#     """
#     Gera uma aba de comparação entre o supermercado mais barato e os outros dois mais baratos.
#     """
#     # 1. Identifica os 3 supermercados com a menor cesta total
#     custos_totais_mercados = {}
#     for nome_completo, data in mercados_data.items():
#         custo_total = data['df'].iloc[:, 0].sum() + data['frete']
#         custos_totais_mercados[nome_completo] = custo_total
    
#     top_3_mercados = sorted(custos_totais_mercados.items(), key=lambda item: item[1])[:3]
#     top_3_nomes_completos = [item[0] for item in top_3_mercados]
    
#     mercado_melhor_key = top_3_nomes_completos[0]
#     mercado_melhor_curto = mercados_data[mercado_melhor_key]['nome_curto']
    
#     precos_comparativos = pd.DataFrame()
#     fretes = {}
    
#     for nome_completo in top_3_nomes_completos:
#         data = mercados_data[nome_completo]
#         precos_comparativos[data['nome_curto']] = data['df'].iloc[:, 0]
#         fretes[data['nome_curto']] = data['frete']
    
#     precos_comparativos.loc['Frete'] = pd.Series(fretes)

#     economias = {}
    
#     # Frete do melhor mercado para ser usado na comparação
#     frete_melhor_mercado = fretes[mercado_melhor_curto]
    
#     # Preços do melhor mercado para ser usado como base de comparação
#     precos_melhor_mercado = precos_comparativos[mercado_melhor_curto]

#     for mercado_curto_atual in precos_comparativos.columns:
#         if mercado_curto_atual == mercado_melhor_curto:
#             economias[mercado_curto_atual] = 0
#             continue
            
#         frete_mercado_atual = fretes[mercado_curto_atual]
#         soma_das_diferencas = 0
        
#         # Compara cada produto do mercado atual com o melhor mercado
#         for produto in precos_melhor_mercado.index:
#             try:
#                 preco_melhor = precos_melhor_mercado.loc[produto]
#                 preco_atual = precos_comparativos.loc[produto, mercado_curto_atual]
                
#                 if pd.notna(preco_melhor) and pd.notna(preco_atual):
#                     soma_das_diferencas += (preco_atual - preco_melhor)
#             except KeyError:
#                 continue

#         # A economia total é a soma das diferenças de preço - frete
#         economias[mercado_curto_atual] = frete_mercado_atual + soma_das_diferencas

#     precos_comparativos.loc['Total Economizado'] = pd.Series(economias)
    
#     # Escreve os dados na planilha
#     nome_aba = 'Comparativo Dinâmico'
#     precos_comparativos.index.name = 'Produto'
#     precos_comparativos.to_excel(writer, sheet_name=nome_aba, index=True)
    
#     # Aplica a formatação condicional e de valores
#     workbook = writer.book
#     worksheet = workbook[nome_aba]

#     verde_fill = PatternFill(start_color='63BE7B', end_color='63BE7B', fill_type='solid')
#     azul_fill = PatternFill(start_color='4472C4', end_color='4472C4', fill_type='solid')
    
#     num_mercados = len(precos_comparativos.columns)
    
#     # Formatação das colunas de preço
#     for col_idx in range(2, num_mercados + 2):
#         for row_idx in range(2, len(precos_comparativos.index) + 2):
#             cell = worksheet.cell(row=row_idx, column=col_idx)
#             cell.number_format = '"R$" #,##0.00'
    
#     # Preenche a célula do frete do melhor mercado com a cor verde
#     frete_row_index = list(precos_comparativos.index).index('Frete') + 2
#     worksheet.cell(row=frete_row_index, column=top_3_nomes_completos.index(mercado_melhor_key) + 2).fill = verde_fill
    
#     # Aumenta a largura das colunas
#     for col in worksheet.columns:
#         max_length = 0
#         column_letter = get_column_letter(col[0].column)
#         for cell in col:
#             try:
#                 print(cell.value)
#                 if len(str(cell.value)) > max_length:
#                     max_length = len(str(cell.value))
#             except:
#                 pass
#         adjusted_width = (max_length + 2)
#         worksheet.column_dimensions[column_letter].width = adjusted_width

#     print("Aba 'Comparativo Dinâmico' gerada com sucesso e formatada.")

# def gerar_aba_comparacao_parcial(writer, mercados_data, mercado1_key, mercado2_key):
#     """
#     Gera uma aba de comparação entre dois mercados específicos.
#     """
#     try:
#         # Identifica os dados dos dois mercados
#         mercado1_data = mercados_data[mercado1_key]
#         mercado2_data = mercados_data[mercado2_key]
        
#         mercado1_curto = mercado1_data['nome_curto']
#         mercado2_curto = mercado2_data['nome_curto']
        
#         # Cria um DataFrame com os preços dos dois mercados
#         df_comparacao = pd.DataFrame({
#             mercado1_curto: mercado1_data['df'].iloc[:, 0],
#             mercado2_curto: mercado2_data['df'].iloc[:, 0]
#         })
        
#         # CONVERSÃO PARA NÚMERO PARA GARANTIR CÁLCULOS CORRETOS
#         df_comparacao[mercado1_curto] = pd.to_numeric(df_comparacao[mercado1_curto], errors='coerce')
#         df_comparacao[mercado2_curto] = pd.to_numeric(df_comparacao[mercado2_curto], errors='coerce')

#         # Calcula a diferença de preço para cada produto
#         df_comparacao['Diferença'] = df_comparacao[mercado2_curto] - df_comparacao[mercado1_curto]
        
#         # Adiciona a linha de frete
#         df_comparacao.loc['Frete'] = [mercado1_data['frete'], mercado2_data['frete'], mercado2_data['frete'] - mercado1_data['frete']]
        
#         # Calcula o total da diferença de preço
#         total_diferenca = df_comparacao.loc[df_comparacao.index != 'Frete', 'Diferença'].sum()
#         df_comparacao.loc['Total Diferença'] = ['', '', total_diferenca]

#         # NOVO: Adiciona a linha de 'Total Economizado'
#         total_cesta_m1 = df_comparacao.loc[df_comparacao.index != 'Frete', mercado1_curto].sum() + mercado1_data['frete']
#         total_cesta_m2 = df_comparacao.loc[df_comparacao.index != 'Frete', mercado2_curto].sum() + mercado2_data['frete']
#         economia_total = total_cesta_m1 - total_cesta_m2  # Ajuste na ordem da subtração
#         df_comparacao.loc['Total Economizado'] = ['', '', economia_total]
        
#         # Escreve a planilha
#         nome_aba = f"{mercado1_curto} - {mercado2_curto}"
#         df_comparacao.to_excel(writer, sheet_name=nome_aba, index=True)
        
#         # Aplica a formatação
#         workbook = writer.book
#         worksheet = workbook[nome_aba]
        
#         for col in worksheet.columns:
#             max_length = 0
#             column_letter = get_column_letter(col[0].column)
#             for cell in col:
#                 try:
#                     if len(str(cell.value)) > max_length:
#                         max_length = len(str(cell.value))
#                 except:
#                     pass
#             adjusted_width = (max_length + 2)
#             worksheet.column_dimensions[column_letter].width = adjusted_width
            
#         for row_idx, produto in enumerate(df_comparacao.index, start=2):
#             cell_m1 = worksheet.cell(row=row_idx, column=2)
#             cell_m2 = worksheet.cell(row=row_idx, column=3)
            
#             if pd.notna(cell_m1.value) and pd.notna(cell_m2.value):
#                 if cell_m1.value < cell_m2.value:
#                     cell_m1.fill = PatternFill(start_color='63BE7B', end_color='63BE7B', fill_type='solid')
#                 elif cell_m2.value < cell_m1.value:
#                     cell_m2.fill = PatternFill(start_color='63BE7B', end_color='63BE7B', fill_type='solid')

#         print(f"Aba '{nome_aba}' gerada com sucesso.")

#     except Exception as e:
#         print(f"Erro ao gerar aba '{mercado1_curto} - {mercado2_curto}': {e}. Linha do erro: {e.__traceback__.tb_lineno}")


# def gerar_excel_final(analise_final_df, mercados_data, nome_arquivo='analise_supermercados_final5.xlsx'):
#     """
#     Gera um arquivo Excel com abas para cada supermercado recomendado,
#     um resumo comparativo e comparações em pares.
#     """
#     with pd.ExcelWriter(nome_arquivo, engine='openpyxl') as writer:
        
#         # 1. Identifica os 3 supermercados com menor custo total
#         custos_totais_mercados = {
#             nome: data['df'].iloc[:, 0].sum() + data['frete']
#             for nome, data in mercados_data.items()
#         }
#         top_3_mercados = sorted(custos_totais_mercados.items(), key=lambda item: item[1])[:3]
#         top_3_nomes_completos = [item[0] for item in top_3_mercados]

#         # 2. Gera a aba principal de comparativo dinâmico (original)
#         gerar_aba_comparacao_dinamica(writer, mercados_data, analise_final_df)
        
#         # 3. Gera as abas de comparação em pares para os 3 mais baratos
#         for mercado1_key, mercado2_key in combinations(top_3_nomes_completos, 2):
#             gerar_aba_comparacao_parcial(writer, mercados_data, mercado1_key, mercado2_key)

#         # 4. Gera as abas individuais para cada supermercado recomendado
#         mercados_recomendados_keys = analise_final_df['Supermercado Recomendado'].unique()
#         print('teste')
#         mercados_recomendados_ordenados = sorted(
#             mercados_recomendados_keys,
#             key=lambda x: int(x.split(' ')[1]) if 'TOP' in x else float('inf')
#         )
#         print('teste')
        
#         for nome_mercado_completo in mercados_recomendados_ordenados:
#             data = mercados_data[nome_mercado_completo]
#             df_mercado = data['df'].copy()
#             frete = data['frete']
            
#             df_mercado.rename(columns={df_mercado.columns[0]: 'Preço Unitário'}, inplace=True)
            
#             df_mercado['Custo Total (c/ Frete)'] = df_mercado['Preço Unitário'] + frete
            
#             custos_totais_com_nomes_curtos = pd.DataFrame({
#                 d['nome_curto']: d['df'].iloc[:, 0]
#                 for d in mercados_data.values()
#             })
#             economias = {}
#             for produto in custos_totais_com_nomes_curtos.index:
#                 custos_ordenados = custos_totais_com_nomes_curtos.loc[produto].sort_values()
#                 custo_melhor = custos_ordenados.iloc[0]
#                 try:
#                     custo_segundo_melhor = custos_ordenados.iloc[1]
#                     economia_calculada = custo_segundo_melhor - custo_melhor
#                     economias[produto] = economia_calculada
#                 except IndexError:
#                     economias[produto] = 0
            
#             df_mercado['Economia (vs 2º Melhor)'] = df_mercado.index.map(economias.get).fillna(0)

#             total_economizado = df_mercado['Economia (vs 2º Melhor)'].sum()
#             df_frete = pd.DataFrame({'Frete': frete}, index=['Frete'])
#             df_total = pd.DataFrame({'Economia (vs 2º Melhor)': total_economizado}, index=['Total Economizado'])
            
#             df_final = df_mercado[['Preço Unitário', 'Custo Total (c/ Frete)', 'Economia (vs 2º Melhor)']].copy()
#             df_final.loc['Frete', 'Preço Unitário'] = frete
#             df_final.loc['Total Economizado', 'Preço Unitário'] = None
#             df_final.loc['Frete', 'Custo Total (c/ Frete)'] = None
#             df_final.loc['Total Economizado', 'Custo Total (c/ Frete)'] = None
            
#             nome_aba = data['nome_curto']
#             df_final.to_excel(writer, sheet_name=nome_aba, index=True)
#             print(f"Aba '{nome_aba}' gerada com sucesso. Total economizado: R$ {total_economizado:.2f}")


# # --- Seu código para execução ---

# arquivos_do_excel = "Resultados\\analise_supermercados_2025-08-27.xlsx"

# try:
#     analise_completa, mercados_dados = realizar_analise_reversa_acumulativa(arquivos_do_excel)
#     gerar_excel_final(analise_completa, mercados_dados)
    
# except FileNotFoundError:
#     print(f"Erro: O arquivo '{arquivos_do_excel}' não foi encontrado.")
# except Exception as e:
#     print(f"Ocorreu um erro: {e}")

import pandas as pd
import io
from openpyxl.styles import PatternFill
from openpyxl.utils import get_column_letter
from itertools import combinations

def realizar_analise_reversa_acumulativa(file_path):
    """
    Realiza a análise de preços de produtos lendo diretamente de um arquivo Excel,
    encontrando o item mais barato em cada mercado para a cesta de compras.
    """
    mercados_data = {}
    
    excel_file = pd.ExcelFile(file_path)
    sheet_names = excel_file.sheet_names
    
    for sheet_name in sheet_names:
        df = pd.read_excel(file_path, sheet_name=sheet_name)
        
        # A lógica para extrair os dados de cada aba permanece a mesma
        df.set_index('Produto', inplace=True)
        nome_mercado_curto = df.columns[0]
        
        # Converte o frete e a coluna de preços para numérico ao ler
        frete = pd.to_numeric(df.loc['Frete', nome_mercado_curto], errors='coerce')
        df[nome_mercado_curto] = pd.to_numeric(df[nome_mercado_curto], errors='coerce')
        
        df_produtos = df.drop(index=['Frete', 'Valor Mínimo', 'Valor Total', 'Total Baratos + Frete'], errors='ignore')

        mercados_data[sheet_name] = {
            'df': df_produtos,
            'frete': frete,
            'nome_curto': nome_mercado_curto
        }

    resultados_finais = []
    
    custos_totais = pd.DataFrame()
    for nome_arquivo_completo, data in mercados_data.items():
        df_temp = data['df'].copy()
        custo_total = df_temp.iloc[:, 0].sum()
        if pd.notna(data['frete']):
            custo_total += data['frete']
        df_temp['Custo Total'] = custo_total
        custos_totais[nome_arquivo_completo] = df_temp['Custo Total']
    
    for produto in custos_totais.index:
        custos_ordenados = custos_totais.loc[produto].dropna().sort_values()
        
        if not custos_ordenados.empty:
            mercado_mais_barato_key = custos_ordenados.index[0]
            preco_produto = mercados_data[mercado_mais_barato_key]['df'].loc[produto].iloc[0]
            frete = mercados_data[mercado_mais_barato_key]['frete']
            custo_total = preco_produto + frete
            
            resultados_finais.append({
                'Produto': produto,
                'Supermercado Recomendado': mercado_mais_barato_key,
                'Preço do Produto': preco_produto,
                'Frete': frete,
                'Custo Total': custo_total,
            })
            
    return pd.DataFrame(resultados_finais).sort_values(by='Produto'), mercados_data

def gerar_aba_comparacao_dinamica(writer, mercados_data, analise_final_df):
    """
    Gera uma aba de comparação entre o supermercado mais barato e os outros dois mais baratos.
    """
    # 1. Identifica os 3 supermercados com a menor cesta total
    custos_totais_mercados = {}
    for nome_completo, data in mercados_data.items():
        custo_total = data['df'].iloc[:, 0].sum() + data['frete']
        custos_totais_mercados[nome_completo] = custo_total
    
    top_3_mercados = sorted(custos_totais_mercados.items(), key=lambda item: item[1])[:3]
    top_3_nomes_completos = [item[0] for item in top_3_mercados]
    
    mercado_melhor_key = top_3_nomes_completos[0]
    mercado_melhor_curto = mercados_data[mercado_melhor_key]['nome_curto']
    
    precos_comparativos = pd.DataFrame()
    fretes = {}
    
    for nome_completo in top_3_nomes_completos:
        data = mercados_data[nome_completo]
        precos_comparativos[data['nome_curto']] = data['df'].iloc[:, 0]
        fretes[data['nome_curto']] = data['frete']
    
    precos_comparativos.loc['Frete'] = pd.Series(fretes)

    economias = {}
    
    # Frete do melhor mercado para ser usado na comparação
    frete_melhor_mercado = fretes[mercado_melhor_curto]
    
    # Preços do melhor mercado para ser usado como base de comparação
    precos_melhor_mercado = precos_comparativos[mercado_melhor_curto]

    for mercado_curto_atual in precos_comparativos.columns:
        if mercado_curto_atual == mercado_melhor_curto:
            economias[mercado_curto_atual] = 0
            continue
            
        frete_mercado_atual = fretes[mercado_curto_atual]
        soma_das_diferencas = 0
        
        # Compara cada produto do mercado atual com o melhor mercado
        for produto in precos_melhor_mercado.index:
            try:
                preco_melhor = precos_melhor_mercado.loc[produto]
                preco_atual = precos_comparativos.loc[produto, mercado_curto_atual]
                
                if pd.notna(preco_melhor) and pd.notna(preco_atual):
                    soma_das_diferencas += (preco_atual - preco_melhor)
            except KeyError:
                continue

        # A economia total é a soma das diferenças de preço - frete
        economias[mercado_curto_atual] = frete_mercado_atual + soma_das_diferencas

    precos_comparativos.loc['Total Economizado'] = pd.Series(economias)
    
    # Escreve os dados na planilha
    nome_aba = 'Comparativo Dinâmico'
    precos_comparativos.index.name = 'Produto'
    precos_comparativos.to_excel(writer, sheet_name=nome_aba, index=True)
    
    # Aplica a formatação condicional e de valores
    workbook = writer.book
    worksheet = workbook[nome_aba]

    verde_fill = PatternFill(start_color='63BE7B', end_color='63BE7B', fill_type='solid')
    azul_fill = PatternFill(start_color='4472C4', end_color='4472C4', fill_type='solid')
    
    num_mercados = len(precos_comparativos.columns)
    
    # Formatação das colunas de preço
    for col_idx in range(2, num_mercados + 2):
        for row_idx in range(2, len(precos_comparativos.index) + 2):
            cell = worksheet.cell(row=row_idx, column=col_idx)
            cell.number_format = '"R$" #,##0.00'
    
    # Preenche a célula do frete do melhor mercado com a cor verde
    frete_row_index = list(precos_comparativos.index).index('Frete') + 2
    worksheet.cell(row=frete_row_index, column=top_3_nomes_completos.index(mercado_melhor_key) + 2).fill = verde_fill
    
    # Aumenta a largura das colunas
    for col in worksheet.columns:
        max_length = 0
        column_letter = get_column_letter(col[0].column)
        for cell in col:
            try:
                if len(str(cell.value)) > max_length:
                    max_length = len(str(cell.value))
            except:
                pass
        adjusted_width = (max_length + 2)
        worksheet.column_dimensions[column_letter].width = adjusted_width

    print("Aba 'Comparativo Dinâmico' gerada com sucesso e formatada.")

def gerar_aba_comparacao_parcial(writer, mercados_data, mercado1_key, mercado2_key):
    """
    Gera uma aba de comparação entre dois mercados específicos.
    """
    # Identifica os dados dos dois mercados
    mercado1_data = mercados_data[mercado1_key]
    mercado2_data = mercados_data[mercado2_key]
    
    mercado1_curto = mercado1_data['nome_curto']
    mercado2_curto = mercado2_data['nome_curto']
    
    # Cria um DataFrame com os preços dos dois mercados
    df_comparacao = pd.DataFrame({
        mercado1_curto: mercado1_data['df'].iloc[:, 0],
        mercado2_curto: mercado2_data['df'].iloc[:, 0]
    })
    
    # CONVERSÃO PARA NÚMERO PARA GARANTIR CÁLCULOS CORRETOS
    df_comparacao[mercado1_curto] = pd.to_numeric(df_comparacao[mercado1_curto], errors='coerce')
    df_comparacao[mercado2_curto] = pd.to_numeric(df_comparacao[mercado2_curto], errors='coerce')

    # Calcula a diferença de preço para cada produto
    df_comparacao['Diferença'] = df_comparacao[mercado2_curto] - df_comparacao[mercado1_curto]
    
    # Adiciona a linha de frete
    df_comparacao.loc['Frete'] = [mercado1_data['frete'], mercado2_data['frete'], mercado2_data['frete'] - mercado1_data['frete']]
    
    # Calcula o total da diferença de preço
    total_diferenca = df_comparacao.loc[df_comparacao.index != 'Frete', 'Diferença'].sum()
    df_comparacao.loc['Total Diferença'] = ['', '', total_diferenca]

    # NOVO: Adiciona a linha de 'Total Economizado'
    # Baseado na sua nova instrução: Total Diferença - Frete (diferença)
    frete_diferenca = df_comparacao.loc['Frete', 'Diferença']
    economia_total = total_diferenca - frete_diferenca
    df_comparacao.loc['Total Economizado'] = ['', '', economia_total]
    
    # Escreve a planilha
    nome_aba = f"{mercado1_curto} - {mercado2_curto}"
    df_comparacao.to_excel(writer, sheet_name=nome_aba, index=True)
    
    # Aplica a formatação
    workbook = writer.book
    worksheet = workbook[nome_aba]
    
    for col in worksheet.columns:
        max_length = 0
        column_letter = get_column_letter(col[0].column)
        for cell in col:
            try:
                if len(str(cell.value)) > max_length:
                    max_length = len(str(cell.value))
            except:
                pass
        adjusted_width = (max_length + 2)
        worksheet.column_dimensions[column_letter].width = adjusted_width
        
    for row_idx, produto in enumerate(df_comparacao.index, start=2):
        cell_m1 = worksheet.cell(row=row_idx, column=2)
        cell_m2 = worksheet.cell(row=row_idx, column=3)
        
        if pd.notna(cell_m1.value) and pd.notna(cell_m2.value):
            if cell_m1.value < cell_m2.value:
                cell_m1.fill = PatternFill(start_color='63BE7B', end_color='63BE7B', fill_type='solid')
            elif cell_m2.value < cell_m1.value:
                cell_m2.fill = PatternFill(start_color='63BE7B', end_color='63BE7B', fill_type='solid')

    print(f"Aba '{nome_aba}' gerada com sucesso.")


def gerar_excel_final(analise_final_df, mercados_data, nome_arquivo='analise_supermercados_final5.xlsx'):
    """
    Gera um arquivo Excel com abas para cada supermercado recomendado,
    um resumo comparativo e comparações em pares.
    """
    with pd.ExcelWriter(nome_arquivo, engine='openpyxl') as writer:
        
        # 1. Identifica os 3 supermercados com menor custo total
        custos_totais_mercados = {
            nome: data['df'].iloc[:, 0].sum() + data['frete']
            for nome, data in mercados_data.items()
        }
        top_3_mercados = sorted(custos_totais_mercados.items(), key=lambda item: item[1])[:3]
        top_3_nomes_completos = [item[0] for item in top_3_mercados]

        # 2. Gera a aba principal de comparativo dinâmico (original)
        gerar_aba_comparacao_dinamica(writer, mercados_data, analise_final_df)
        
        # 3. Gera as abas de comparação em pares para os 3 mais baratos
        for mercado1_key, mercado2_key in combinations(top_3_nomes_completos, 2):
            gerar_aba_comparacao_parcial(writer, mercados_data, mercado1_key, mercado2_key)

        # 4. Gera as abas individuais para cada supermercado recomendado
        mercados_recomendados_keys = analise_final_df['Supermercado Recomendado'].unique()
        mercados_recomendados_ordenados = sorted(
            mercados_recomendados_keys,
            key=lambda x: int(x.split(' ')[1]) if 'TOP' in x else float('inf')
        )
        
        for nome_mercado_completo in mercados_recomendados_ordenados:
            data = mercados_data[nome_mercado_completo]
            df_mercado = data['df'].copy()
            frete = data['frete']
            
            df_mercado.rename(columns={df_mercado.columns[0]: 'Preço Unitário'}, inplace=True)
            
            df_mercado['Custo Total (c/ Frete)'] = df_mercado['Preço Unitário'] + frete
            
            custos_totais_com_nomes_curtos = pd.DataFrame({
                d['nome_curto']: d['df'].iloc[:, 0]
                for d in mercados_data.values()
            })
            economias = {}
            for produto in custos_totais_com_nomes_curtos.index:
                custos_ordenados = custos_totais_com_nomes_curtos.loc[produto].sort_values()
                custo_melhor = custos_ordenados.iloc[0]
                try:
                    custo_segundo_melhor = custos_ordenados.iloc[1]
                    economia_calculada = custo_segundo_melhor - custo_melhor
                    economias[produto] = economia_calculada
                except IndexError:
                    economias[produto] = 0
            
            df_mercado['Economia (vs 2º Melhor)'] = df_mercado.index.map(economias.get).fillna(0)

            total_economizado = df_mercado['Economia (vs 2º Melhor)'].sum()
            df_frete = pd.DataFrame({'Frete': frete}, index=['Frete'])
            df_total = pd.DataFrame({'Economia (vs 2º Melhor)': total_economizado}, index=['Total Economizado'])
            
            df_final = df_mercado[['Preço Unitário', 'Custo Total (c/ Frete)', 'Economia (vs 2º Melhor)']].copy()
            df_final.loc['Frete', 'Preço Unitário'] = frete
            df_final.loc['Total Economizado', 'Preço Unitário'] = None
            df_final.loc['Frete', 'Custo Total (c/ Frete)'] = None
            df_final.loc['Total Economizado', 'Custo Total (c/ Frete)'] = None
            
            nome_aba = data['nome_curto']
            df_final.to_excel(writer, sheet_name=nome_aba, index=True)
            print(f"Aba '{nome_aba}' gerada com sucesso. Total economizado: R$ {total_economizado:.2f}")


# --- Seu código para execução ---

arquivos_do_excel = "Resultados\\analise_supermercados_2025-08-27.xlsx"

try:
    analise_completa, mercados_dados = realizar_analise_reversa_acumulativa(arquivos_do_excel)
    gerar_excel_final(analise_completa, mercados_dados)
    
except FileNotFoundError:
    print(f"Erro: O arquivo '{arquivos_do_excel}' não foi encontrado.")
except Exception as e:
    print(f"Ocorreu um erro: {e}")